    ...
    if (round_index == 0) {
        ...
        for (iter = map_id_queue[party_id].begin(); iter != map_id_queue[party_id].end(); ) {
            ...
            if (ctx_ptr->IsFinished()) {
                std::cout << "<== Finished , Party " << ctx_ptr->minimal_sign_key_.local_party_.party_id_ << std::endl;
            }
...
void print_sign_key_info(vector<Context*> ctx_arr){
    ...
    for(size_t i = 0; i < ctx_arr.size(); ++i){
        ...
        string json_str;
        EXPECT_TRUE(ctx_arr[i]->minimal_sign_key_.ToJsonString(json_str));
        std::cout << ctx_arr[i]->minimal_sign_key_.local_party_.party_id_ << ": \n    "
                  << "  - " << ctx_arr[i]->minimal_sign_key_.X_.Inspect() << ": \n    "
                  << "  - " << json_str << std::endl;
    }
}

void print_sign_key_bas64_arr(vector<Context*> ctx_arr){
    std::cout << "Aggregated Public Key: " << ctx_arr[0]->minimal_sign_key_.X_.Inspect() << std::endl;
    std::cout << "Vault key(base64) of co-signer1, co-signer2 and owner: " << std::endl;
    std::cout << "{ " << std::endl;
    for(size_t i = 0; i < ctx_arr.size(); ++i){
        // Base64 format of sign_key key
        string base64;
        EXPECT_TRUE(ctx_arr[i]->minimal_sign_key_.ToBase64(base64));
        if(i != ctx_arr.size() - 1){
...
void testKeyGen_t_n(CurveType curve_type) {
    ...
    for (int round = 0; round <= 3; ++round) {
        ...
        for (int i = 0; i < 3; ++i) {
            std::cout << "<== Round " << round << ", " << ctx_arr[i]->minimal_sign_key_.local_party_.party_id_  << std::endl;
